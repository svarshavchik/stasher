...
private:
	...
	class getRequestReceivedObj;
	class updateRequestReceivedObj;
...
// The first thing that the "updateobject" method needs to do is get the
// object's existing contents, by sending a get_request. This is the destructor
// callback for get_request's mcguffin.

class updateThreadObj::getRequestReceivedObj
	: public x::destroyCallbackFlagObj {

	// The original update thread.
	x::ref<updateThreadObj> me;

	// The original update request
	x::ref<updateRequestObj> req;

	// The original update request's mcguffin
	x::ptr<x::obj> mcguffin;

	// The get_request for the existing contents.
	stasher::getrequest getreq;

public:
	getRequestReceivedObj(const x::ref<updateThreadObj> &meArg,
			      const x::ref<updateRequestObj> &reqArg,
			      const x::ptr<x::obj> &mcguffinArg,
			      const stasher::getrequest &getreqArg)
		: me(meArg), req(reqArg), mcguffin(mcguffinArg),
		  getreq(getreqArg)
	{
	}

	~getRequestReceivedObj() noexcept
	{
	}

	// When the destructor callback gets invoked, the get_request is
	// complete, and the processcontents() message gets sent to the thread.
	void destroyed() noexcept override
	{
		me->processcontents(getreq->getmsg()->objects,
				    req, mcguffin);
	}
};
				...
				const x::ptr<x::obj> &msg_mcguffin)
{
	...
	std::pair<stasher::getrequest, stasher::client::base::request>
		pending_req=client->get_request(req);

	// Set up a destructor callback on the get_request's mcguffin, which
	// sends a "processcontents" message, when the get_request completes.
	pending_req.second->mcguffin()
		->addOnDestroy(x::ref<getRequestReceivedObj>
			       ::create(x::ref<updateThreadObj>(this),
					msg_request,
					msg_mcguffin,
					pending_req.first));
}

// The processcontents message results in a put_request. This is the destructor
// callback for put_request's mcguffin.
//
// This is similar to getRequestReceivedObj.

class updateThreadObj::updateRequestReceivedObj
	: public x::destroyCallbackFlagObj {
	...
	// The get_request for the existing contents.
	stasher::putrequest putreq;

public:
	updateRequestReceivedObj(const x::ref<updateThreadObj> &meArg,
				 const x::ref<updateRequestObj> &reqArg,
				 const x::ptr<x::obj> &mcguffinArg,
				 const stasher::putrequest &putreqArg)
		: me(meArg), req(reqArg), mcguffin(mcguffinArg),
		  putreq(putreqArg)
	{
	}

	~updateRequestReceivedObj() noexcept
	{
	}

	// When the destructor callback gets invoked, the put_request is
	// complete, and the processupdate() message gets sent to the thread.

	void destroyed() noexcept override
	{
		me->processupdate(putreq->getmsg(), req, mcguffin);
	}
};
...
void updateThreadObj::dispatch(const processcontents_msg &msg)
{
	...
	std::pair<stasher::putrequest, stasher::client::base::request>
		pending_req=client->
		put_request(iter == msg.contents->end()
			    ? msg.request->update(x::fdptr(), x::uuid())
			    : msg.request->update(iter->second.fd,
						  iter->second.uuid));

	// Set up a destructor callback on the put_request's mcguffin, which
	// sends a "processupdate" message, when the get_request completes.
	pending_req.second->mcguffin()
		->addOnDestroy(x::ref<updateRequestReceivedObj>
			       ::create(x::ref<updateThreadObj>(this),
					msg.request,
					msg.mcguffin,
					pending_req.first));
